...
if ($resource === 'events' && $_SERVER['REQUEST_METHOD'] === 'GET') {
  ...
  $res = $db->query("
    SELECT 
      id, title, description, category, event_date, start_time,
      price, image_url, isActive, createdAt
    FROM events
    WHERE isActive = 1
...
if ($resource === 'promotions' && $_SERVER['REQUEST_METHOD'] === 'GET') {
  ...
  $res = $db->query("
    SELECT 
      ...
      percent_off, fixed_amount, promo_code, image_url, isActive, createdAt
    FROM promotions
    WHERE isActive = 1
